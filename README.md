...
-   **Local Development Mode (Default)**: When the `JwtAuthority` configuration value is empty, the services operate in a local mode. They use a symmetric key (`DevSymmetricKey`) that is automatically generated by the `init.ps1` script and stored in .NET User Secrets. This allows developers to test secured endpoints without needing to connect to an external identity provider.
-   **Identity Provider (IdP) Mode**: If `JwtAuthority` is set to the URL of an OIDC-compliant identity provider, the services will automatically switch to validating JWTs against that authority. This makes it easy to transition from local development to a production-like environment.
    > **Note on IdP Configuration:** When using an external IdP, the scopes and roles used in the API's authorization policies (e.g., `insurance:read`, `AdminOnly`) must be defined within the IdP. The names must match exactly, as they form the contract between the IdP and the API. The IdP is responsible for issuing JWTs containing these claims based on user permissions.

Authorization is handled using policies based on scopes and roles. The following policies are defined:
...
Before you run the application, you need to configure the necessary secrets (like the database password and JWT settings). You only need to provide your secrets once in a local file, and a script will configure them for both the Docker and non-Docker environments.
...
2.  **Configure secrets**: Open `secrets.local.json` and provide your secrets.
    ```json
    {
      ...
      // Set to your IdP's issuer URL to enable IdP mode.
      "JwtAuthority": "",
      "JwtAudience": "api://insurance-service",
      "JwtIssuer": "https://local-dev-issuer"
    }
    ```
    The `init.ps1` script will use these values to configure the services. See the "Authentication and Authorization" section for more details on the JWT settings.
